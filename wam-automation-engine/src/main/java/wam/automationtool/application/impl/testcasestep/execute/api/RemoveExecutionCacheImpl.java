...
public class RemoveExecutionCacheImpl extends TestCaseStepExecutorBase
  ...
  private void start(
    ...
    try {
      ...
      getWamCacheManager().removeItemFromCache(testCaseStepExecuteRequestDto.getExecutionId());
    } catch (final Exception exception) {
      throw new TestCaseStepExecutionFailException(
          BAD_REQUEST,
          TEST_CASE_STEP_EXECUTION_FAIL_CODE,
          "Failed to clear execution cache: "
              .concat(testCaseStepExecuteRequestDto.getExecutionId()));
    }
  }
}
